...
def invoice_pdf(inv_no: str, customer: str, cust_address: str, cust_contact: str,
    ...
    el.append(info)
    el.append(Spacer(1, 2 * mm))
    addr_lines = "<br/>".join(filter(None, [cust_address, f"Tel: {cust_contact}" if cust_contact else ""]))
    if addr_lines:
        el.append(Paragraph(addr_lines, SMALL))
    el.append(Spacer(1, 5 * mm))
    ...
    t.setStyle(_grid_style(last_bold=True))
    ...
    if notes:
        el.append(Paragraph(f"Notes: {notes}", SMALL))
        el.append(Spacer(1, 6 * mm))

    el.append(_sig_block(["Issued By", "Received By"]))
    el.append(Spacer(1, 4 * mm))
    el.append(Paragraph(f"Generated by CATDAY System - {inv_no}", TINY))
    doc.build(el)
    return rel
